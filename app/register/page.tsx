    ...
    <div>
      <div className="relative">
        <input
          ...
          className={`w-full px-4 py-2.5 pr-11 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all bg-white ${
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          last_name: lastName,
          role: role === "org" ? "coordinator" : "seeker",
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm p-7">
          ...
          {step === 1 && (
            <>
              <h1 className="text-xl font-bold text-foreground mb-2 text-center">
                Як ти плануєш використовувати Моживо?
              </h1>
              <p className="text-sm text-muted text-center mb-6">
                Обери роль — від цього залежатимуть можливості акаунту
              </p>
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted mt-1 leading-tight">
                      Студент, активіст, волонтер
                    </p>
                  </div>
                </button>
                <button
                  onClick={() => selectRole("org")}
                  className="group flex flex-col items-center gap-3 p-5 border-2 border-border rounded-2xl hover:border-primary hover:bg-primary-light transition-all duration-200"
                >
                  <span className="text-4xl">🏢</span>
                  <div className="text-center">
                    <p className="font-semibold text-foreground group-hover:text-primary transition-colors text-sm">
                      Представляю організацію
                    </p>
                    <p className="text-xs text-muted mt-1 leading-tight">
                      НГО, фонд, університет
                    </p>
                  </div>
                </button>
              </div>
            ...
            <>
              <div className="flex items-center gap-3 mb-5">
                ...
                <h1 className="text-xl font-bold text-foreground">
                  {role === "org" ? "Про організацію" : "Створи акаунт"}
                </h1>
              </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  <Field label="Назва організації" error={errors.orgName}>
                    <input
                      ...
                      className={`w-full px-4 py-2.5 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all bg-white ${
                        errors.orgName ? "border-red-400" : "border-border"
                      }`}
                    />
                  </Field>
                )}
                ...
                {role === "org" && (
                  <Field label="Тип організації" error={errors.orgType}>
                    <select
                      ...
                      className={`w-full px-4 py-2.5 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all bg-white ${
                        errors.orgType ? "border-red-400" : "border-border"
                      }`}
                    >
                ...
                <Field
                  label={role === "org" ? "Контактна особа" : "Ім'я та прізвище"}
                  error={errors.name}
                >
                  <input
                    type="text"
          ...
          {step === 2 && status === "success" && (
            <div className="text-center py-8">
              ...
              <p className="font-bold text-foreground text-lg">Акаунт створено!</p>
              <p className="text-sm text-muted mt-2 leading-relaxed">
                Перевір email — ми надіслали підтвердження.<br />Після підтвердження ти зможеш увійти.
              </p>
            </div>
          )}
          ...
          {/* Server error */}
          {serverError && (
            <p className="text-xs text-red-500 text-center mt-2">{serverError}</p>
          )}
        </div>
