...
export async function generateMetadata({
  ...
  const org = orgsBySlug[params.slug];
  if (!org) return {};
  return {
    title: org.name,
    description: org.description.slice(0, 160),
    alternates: { canonical: `https://mozhyvo.ua/organizations/${org.slug}` },
    openGraph: {
      title: org.name,
      description: org.description.slice(0, 160),
      url: `https://mozhyvo.ua/organizations/${org.slug}`,
    },
  };
}
...
export default function OrgProfilePage({ params }: { params: { slug: string } }) {
  const org = orgsBySlug[params.slug];
  if (!org) notFound();

  const orgOpps = opportunities.filter((o) => orgNameToSlug[o.org] === org.slug);

  const activeSocials = Object.entries(org.socials).filter(([, url]) => !!url) as [
    string,
    string,
  ][];
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${org.coverFrom} 0%, ${org.coverTo} 100%)`,
        }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="-mt-12 mb-8 flex items-end gap-5">
          {/* Logo */}
          <div
            className={`w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center text-white text-xl sm:text-2xl font-black flex-shrink-0 border-4 border-white shadow-xl ${org.logoColor}`}
          >
            {org.logoInitials}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-muted-bg text-muted text-xs font-medium">
                {org.typeName}
              </span>
          ...
          {org.website && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-border text-sm font-semibold text-foreground hover:border-primary hover:text-primary transition-all duration-150 self-start flex-shrink-0"
            >
        ...
        <div className="flex flex-wrap gap-6 pb-8 mb-8 border-b border-border">
          <div>
            <p className="text-2xl font-black text-primary leading-none">{org.stats.opportunities}</p>
            <p className="text-xs text-muted mt-1">можливостей</p>
          </div>
          <div className="w-px bg-border" />
          <div>
            <p className="text-2xl font-black text-primary leading-none">{org.stats.countries}</p>
            <p className="text-xs text-muted mt-1">{org.stats.countries === 1 ? "країна" : "країн"}</p>
          </div>
          {org.stats.participants && (
            <>
              <div className="w-px bg-border" />
              <div>
                <p className="text-2xl font-black text-primary leading-none">{org.stats.participants}</p>
                <p className="text-xs text-muted mt-1">учасників</p>
              </div>
            </>
          )}
          {org.verified && (
            <>
              <div className="w-px bg-border" />
              <div>
                <p className="text-2xl font-black text-blue-600 leading-none">✓</p>
                <p className="text-xs text-muted mt-1">верифіковано</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-16">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section>
              ...
              <p className="text-muted leading-relaxed text-sm">{org.description}</p>
            </section>
            ...
            {/* Tags */}
            <section>
              <h2 className="text-lg font-bold text-foreground mb-3">Теги</h2>
              <div className="flex flex-wrap gap-2">
                {org.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/opportunities?q=${encodeURIComponent(tag)}`}
                    className="px-3 py-1 rounded-full bg-muted-bg text-muted text-xs font-medium hover:bg-primary-light hover:text-primary transition-colors"
                  >
                    #{tag}
                  </Link>
                ))}
            ...
            <div className="sticky top-24 space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-border p-5 space-y-4">
                ...
                {org.email && (
                  ...
                  >
                    <div className="w-9 h-9 rounded-xl bg-muted-bg flex items-center justify-center flex-shrink-0 group-hover:bg-primary-light transition-colors">
                    ...
                    {activeSocials.map(([platform, url]) => {
                      ...
                      return (
                        <a
                          ...
                          rel="noreferrer"
                          aria-label={s.label}
                          className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-border text-sm font-medium text-muted transition-all duration-150 ${s.bg}`}
                        >
                          {s.icon}
                          {s.label}
                        </a>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-600 mt-0.5 leading-relaxed">
                        Організація пройшла перевірку автентичності та відповідності стандартам платформи.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-muted mt-1">
                  {orgOpps.length} {orgOpps.length === 1 ? "програма" : orgOpps.length < 5 ? "програми" : "програм"} на платформі
                </p>
              </div>
              <Link
        ...
        {orgOpps.length === 0 && (
          <section className="pb-16">
            <div className="bg-muted-bg rounded-2xl p-10 text-center">
              ...
              <p className="text-sm text-muted mt-1">
                Слідкуй за оновленнями — нові можливості з&apos;являться незабаром
              </p>
            </div>
          </section>
        )}
