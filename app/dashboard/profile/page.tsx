...
function PreviewCard({
  ...
  form: {
    ...
    brandColor: string; focusAreas: string[];
    socials: Record<string, string>;
  };
  initials: string;
  verified: boolean;
}) {
  const hasCover = !!form.coverImage;
    ...
    <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm">
      ...
      <div
        ...
        style={{
          background: hasCover
            ? undefined
            : `linear-gradient(135deg, ${form.brandColor} 0%, ${form.brandColor}bb 100%)`,
        }}
      >
        {hasCover && (
        ...
        {form.focusAreas.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            {form.focusAreas.slice(0, 4).map((a) => (
              <span key={a} className="px-2 py-0.5 rounded-full bg-primary-light text-primary text-[10px] font-medium">
                {a}
              </span>
            ))}
            {form.focusAreas.length > 4 && (
              <span className="px-2 py-0.5 rounded-full bg-muted-bg text-muted text-[10px]">
                +{form.focusAreas.length - 4}
              </span>
            )}
          </div>
...
function ProfileContent() {
  ...
  function readFile(file: File, maxMb: number, onDone: (dataUrl: string) => void) {
    if (file.size > maxMb * 1024 * 1024) { alert(`Файл занадто великий. Максимум — ${maxMb} МБ.`); return; }
  ...
  function handleLogoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f) readFile(f, 2, (url) => set("logo", url));
  }

  function handleCoverChange(e: React.ChangeEvent<HTMLInputElement>) {
    const f = e.target.files?.[0];
    if (f) readFile(f, 5, (url) => set("coverImage", url));
  }
  ...
  function handleCoverDrop(e: React.DragEvent) {
    ...
    if (f && f.type.startsWith("image/")) readFile(f, 5, (url) => set("coverImage", url));
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
        ...
        <div className="lg:col-span-3 flex flex-col gap-5">
          ...
          <Section
            ...
            hint="Обкладинка відображається у верхній частині вашого публічного профілю."
          >
            {/* Cover upload */}
            <div>
              <label className={lbl}>Обкладинка профілю</label>
              <div
                className={`relative rounded-2xl overflow-hidden h-36 cursor-pointer group transition-all ${coverDrag ? "ring-2 ring-primary ring-offset-2" : ""}`}
                style={{
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-all text-center">
                    ...
                    <p className="text-white text-xs font-semibold">Клікніть або перетягніть</p>
                  </div>
                </div>
                <input ref={coverRef} type="file" accept="image/*" className="hidden" onChange={handleCoverChange} />
              </div>
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-muted">JPG, PNG або WebP · до 5 МБ · рекомендовано 1200×400 px</p>
                {form.coverImage && (
                  <button onClick={() => set("coverImage", null)} className="text-xs text-muted hover:text-red-500 transition-colors font-medium">
                    Видалити обкладинку
                  </button>
                )}
              </div>
            ...
            <div className="flex items-start gap-5 pt-1">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted">PNG або JPG, до 2 МБ, рекомендовано 200×200 px</p>
              </div>
            </div>
            ...
            <div className="pt-1">
              <label className={lbl}>
                ...
                <span className="ml-2 text-xs text-muted font-normal">
                  {form.coverImage ? "використовується для логотипу" : "використовується для обкладинки та логотипу"}
                </span>
              </label>
          ...
          <div className="sticky top-24 flex flex-col gap-4">
            ...
            <div className="bg-primary-light rounded-xl p-4 border border-primary/15">
              ...
              <ul className="text-xs text-primary/70 space-y-1.5">
                <li>• Завантажте яскраву обкладинку — вона одразу привертає увагу</li>
                <li>• Чіткий опис збільшує довіру кандидатів</li>
                <li>• Соцмережі показують що організація активна</li>
                <li>• Місія — потужний елемент для залучення відданих кандидатів</li>
              </ul>
            </div>
          </div>
        </div>
