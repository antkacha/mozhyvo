        ...
        <div className="flex-1 flex flex-col items-center justify-center px-5 py-10 lg:py-16 -mt-4 lg:mt-0">
          <div className="w-full max-w-[420px]">
              ...
              {status === "success" ? (
                ...
                <>
                  <button type="button" onClick={handleGoogle} disabled={status === "loading"}
                    className="w-full flex items-center justify-center gap-3 py-2.5 px-4 border border-border rounded-xl text-sm font-medium text-foreground hover:bg-gray-50 transition-all duration-200 mb-4 disabled:opacity-60">
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    <div>
                      ...
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                        placeholder="твій@email.com" required autoComplete="email"
                    ...
                    <button type="submit" disabled={status === "loading" || !canSubmit}
                      ...
                      {status === "loading" ? (
                        <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Входимо...</>
                      ) : "Увійти"}
                    </button>
                  </form>
                </>
            ...
            <p className="text-center text-sm text-muted mt-5">
              Немає акаунту?{" "}
              <Link href="/register" className="text-primary font-medium hover:underline">Зареєструватись</Link>
            </p>
          </div>
        </div>
      </div>
