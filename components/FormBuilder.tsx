...
import { FormQuestion } from "@/hooks/useOrgProjects";

const TYPES: Array<{ value: FormQuestion["type"]; label: string; hint: string }> = [
...
// ── Preview of a single field as applicant would see it ──────────────
    ...
    <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-muted-bg/30">
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          <button onClick={() => setOpen((v) => !v)} className="w-7 h-7 rounded-lg flex items-center justify-center text-muted hover:bg-muted-bg transition-all">
          ...
          <button onClick={onDelete} className="w-7 h-7 rounded-lg flex items-center justify-center text-muted hover:text-red-500 hover:bg-red-50 transition-all">
      ...
      {open && (
        <div className="p-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <input value={q.label} onChange={(e) => onChange({ ...q, label: e.target.value })} placeholder="Наприклад: Розкажіть про ваш досвід..." className={inp} />
          </div>
          ...
          <div>
            ...
            <input value={q.description ?? ""} onChange={(e) => onChange({ ...q, description: e.target.value })} placeholder="Підказка або уточнення для учасника..." className={inp} />
          </div>
          ...
          {(q.type === "text" || q.type === "textarea") && (
            <div>
              ...
              <input value={q.placeholder ?? ""} onChange={(e) => onChange({ ...q, placeholder: e.target.value })} placeholder="Текст всередині поля..." className={inp} />
            </div>
          )}
              ...
              <div className="flex flex-col gap-2">
                {(q.options ?? []).map((opt, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input value={opt} onChange={(e) => updateOption(i, e.target.value)} placeholder={`Варіант ${i + 1}`} className={`${inp} flex-1`} />
                    <button
                ...
                <button onClick={addOption} className="flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary-dark transition-colors py-0.5 pl-6">
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs text-muted">
          {questions.length === 0 ? "Немає питань" : `${questions.length} ${questions.length === 1 ? "питання" : questions.length < 5 ? "питання" : "питань"}`}
        </p>
        {questions.length > 0 && (
          <button
      ...
      {/* Empty state */}
      {questions.length === 0 && (
        <div className="bg-muted-bg/40 rounded-2xl border border-dashed border-border p-10 text-center mb-4">
          ...
          <p className="font-semibold text-foreground text-sm mb-1">Форма порожня</p>
          <p className="text-xs text-muted">Додайте кастомні питання нижче</p>
        </div>
      )}
